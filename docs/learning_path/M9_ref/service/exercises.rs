    ...
    ) -> Result<tonic::Response<Self::StreamExerciseSeriesStream>, tonic::Status>
{
        ...
        let stats =
            crate::api::rest::stats::exercise_stats_view(&pool, user.id, req.exercise_id).await?;
        ...
        let points = stats
            ...
            .map(|r| crate::api::grpc::convert::series_point(req.exercise_id, r))
            .collect::<Vec<_>>();

        // 变流：每个元素套上 Result 外壳（协议要求流元素是 Result<T, Status>）
        Ok(tonic::Response::new(tokio_stream::iter(
            points.into_iter().map(Ok).collect::<Vec<_>>(),
        )))
}
}
