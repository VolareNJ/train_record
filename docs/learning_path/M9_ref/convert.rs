    ...
    fn from(ex: &crate::api::rest::exercises::ExerciseOut) -> Self
{
        Self {
            ...
            last_record_date: ex.last_record_date.clone(),
...
{
        Self {
            // 【关键写法：Option<&T> → Option<U>】
            // as_ref() 把 &Option<T> 变成 Option<&T>（借用，不复制整个结构体），
            // map(From) 只对 Some 里的值做转换，None 原样传下去。
            phase: v.phase.as_ref().map(crate::api::grpc::pb::PhaseBrief::from),
            date: v.date.clone(),
            plan: v.plan.as_ref().map(crate::api::grpc::pb::TodayPlan::from),
        }
        }
        }
