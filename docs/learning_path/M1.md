...
> **重要提示**：当前 `auth.rs` 和 `handlers/auth.rs` 里的函数都是
...
**方案选择（M1 用方案 B）**：函数体内调用 `require_user(state, headers)`，未登录返回重定向。理由：
- 复用第 2 步已实现的 `require_user`，改动最小
- 提取器（方案 A）需要理解 `FromRequestParts` trait，M1 阶段不引入
- M2+ 起页面变多、守卫代码重复，再改造为 `AuthUser` 提取器（见 `structure.md` §5.1.x）
...
- [x] `cargo check` 无错误
- [x] 启动后访问 `/login` 显示登录页（⏳ 依赖第 3 步 main.rs 注册路由 + 第 6 步登录页）
- [x] 用配置的管理员账号登录成功，跳转到首页（⏳ 依赖第 3、5 步）
- [x] 未登录访问 `/` 被重定向到 `/login`（路由守卫生效）（⏳ 依赖第 3 步）
- [x] 登录后 cookie 里的 session 有效；登出后再次访问被拒绝（⏳ 依赖第 3 步）
- [x] 非管理员访问 `/admin/users` 被拒绝（⏳ 依赖第 3 步）
- [x] 管理员能创建新用户（邀请制）（⏳ 依赖第 3 步）
- [x] 理解验证：独立完成 [七、M1 理解验证（填空题）](#七m1-理解验证填空题)，全部答对才算通过

> M1 全部完成：cargo check 零错误；理解验证 5 题批改通过；6 项实测全部通过——
> /login 200、管理员登录 303→首页、未登录 303→/login、登出后失效、非管理员 403、创建用户 303。
> 实测日期：2026-08-05（commit 4a4b35a）。
> 额外收获：AppError 新增 Forbidden 变体（403），区分"未登录(401)"与"无权限(403)"；
> ensure_admin 已存在时静默（常态不报站，仅首次创建打日志）。
