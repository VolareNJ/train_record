...
6. **自动计算保护条件**（M5 §9.1）：自动联动写值要区分"初始回显"与"用户输入"，空输入 ≠ 0
7. **迁移设计约束**（M5 §9.2）：SQLite `ADD COLUMN` 只能加 NULL/带默认值列，新列对老数据 = NULL，模型必须 `Option`
...
## 9. M5 追加：去按钮自动更新 + 动作级备注（vibe coding 复盘）

> M5 做三件事：① 去掉"填入强度"按钮改自动更新；② 新增动作级备注
> `plan_note`；③ AGENTS.md 加编译缓存检查（流程约定，无知识点，不补课）。
> 本文档继续沿用 M4 的复盘格式：只补**跨框架通用**的知识点。
