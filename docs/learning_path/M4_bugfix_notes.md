...
|新列 → 老数据 NULL | 迁移加列后旧行全为 NULL，模型层必须 `Option` | 会 |
...
|版本号破缓存 | 资源 URL 变化 → 强制重新下载，最简可靠方案 | 会 |
...
|枚举缺失的安全处理 | `position`/`find` 的 None 分支必须有明确语义 | 会 |
...
|`COALESCE` 兜底 | NULL → 默认值，SQL 版 `unwrap_or` | 会 |
...
|先查 PID 再 kill | `ps + grep + awk` 精确定位，安全重启 | 会 |
