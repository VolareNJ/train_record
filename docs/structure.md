...
> 需在防火墙/安全组放行该端口，并先启用 TLS（设 `GRPC_TLS_CERT` / `GRPC_TLS_KEY`，
> 生成证书与客户端信任见 `deploy.md` 六；不设则明文，只建议内网）。
...
- HTTPS/域名：本期公网 IP + 端口直连（无域名）。gRPC 侧已支持配置启用 TLS
  （自签证书，M9 收尾，见 `deploy.md` 六）；网页版 HTTPS 已决定暂缓（不买域名，
  代价：PWA 离线不可用，见 `todo.md` §1.10）
