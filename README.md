...
|  训练阶段 (Phase) | 一段训练期归档为一个阶段，可只读回看、重新启用 |
...
> 注意：PWA / Service Worker 只在 **HTTPS 或 localhost** 下可注册。生产当前是
> 明文 HTTP（公网 IP 直连），“离线缓存”在手机上不生效；原因、代价与后续方案见
> `docs/todo.md` §1.10。（本机开发不受影响。）
...
>
> 实测踩坑（2026-09）：`rustup update` 有时会报
...
> 80 端口需要 root 权限。生产环境建议后续改用 systemd 托管（开机自启 + 崩溃重启），模板见 `docs/structure.md` §7。
...
> 不要用裸 `cargo fmt`：stable rustfmt 不认识 `brace_style`，会“忽略选项后照常格式化”，
...
├── src/
│   ├── lib.rs              # 库入口：模块树 + 共享状态 AppState（M9 起 lib + bin 分离）
│   ├── main.rs             # 入口：装配 AppState、注册路由、启动 HTTP + gRPC 两个服务器
...
- **路径写法**：全路径优先（`axum::extract::State`、`crate::models::User`），
  不用 `use` 做便利导入；唯一例外是 **trait 方法调用**所需的最小导入
  （如 `use axum::response::IntoResponse;`，一个 trait 一行）——详见 `AGENTS.md`「路径与导入」
- **禁止 emoji**：代码/注释/文档/提交信息都只用纯文本（状态写“已完成/待实现”，
  清单用 `- [x]`，重点写“注意：”）——详见 `AGENTS.md`「禁止 emoji」
- **0 warning**：`cargo check --all-targets` / `cargo test` 必须无警告；
  挖空练习的 `todo!()` 可临时加 `#[allow(unused)]`，实现完必须删——详见 `AGENTS.md`「0 warning 纪律」
