// ============================================================
// api/rest/mod.rs —— REST API 层模块入口（M8）
// ============================================================
// 【教学说明：本文件在 M9 的目录归并】
...
//   2. 组装 rest::router()（所有 /api/v1/... 路由）
//
// 【教学：REST 层的查询函数是"公共资产"】
// 下面 7 个子模块里，除了 axum handler（收请求、返回 JSON），
// 还躺着一批 `pub(crate)` 的**协议无关查询函数**，例如：
//   phases::phase_out / calc_days      （阶段 → 输出形状 + 坚持天数）
//   exercises::exercise_out            （动作 → 输出形状 + 1RM/最近日期）
//   plans::verify_phase / plan_out     （归属校验 + 计划 → 输出形状）
//   records::today_out / record_out    （今日卡片 + 记录 → 输出形状）
//   stats::calendar_out / exercise_stats_out
