...
//
//  关于“明文传输”：metadata 是明文的，公网暴露必须套 TLS，否则 token 与
//    Login 请求里的密码都会被中间人看到。本项目已支持：设 GRPC_TLS_CERT /
//    GRPC_TLS_KEY 两个环境变量即启用（生成证书与客户端信任见 docs/deploy.md 六）；
//    不设 = 明文，只建议本机/内网（冒烟测试走 127.0.0.1，就是明文）。
