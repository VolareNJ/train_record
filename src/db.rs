...
///
/// 【教学：人为改表会怎样？——信任账本 + 校验指纹】
/// 迁移机制不"检查表结构"，而是靠两样东西：
///   - 账本：_sqlx_migrations 表记录哪些执行过 → 决定跑不跑
///   - 指纹：每个迁移文件的 checksum（校验和）→ 决定敢不敢跑
/// 场景：
///   - 手动改表结构（如加列）→ 机制不知道，照常启动（账本上没这笔账）
///   - 改已执行的迁移文件内容 → checksum 对不上，启动直接报错拦下！
///   - 改 _sqlx_migrations 表本身 → 等于撕账本，被删的迁移会重新执行
/// 正确做法：改表结构永远新增迁移文件（0002_xxx.sql），别手动改表。
///
